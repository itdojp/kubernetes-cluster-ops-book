...
## 読み方ガイド
- まず全体像をつかみたい場合は、第0章→第1章→第10章→第11章の順に読むと、設計・変更・障害対応の主経路を短時間で整理できます。
- etcd バックアップや復旧に不安がある場合は、第3章を先に読み、付録Bのトラブルシュートフローと併読してください。
- 既存クラスタの改善や標準化が目的なら、第9章〜第12章と付録Aを往復しながら、自組織の運用標準へ落とし込む読み方が有効です。
- 基礎の用語や Kubernetes の基本オブジェクトに不安がある場合は、前提書の [Kubernetes入門：PodからIngressまで（基礎と実践）](https://itdojp.github.io/kubernetes-basics-book/) を先に確認してください。
...
## 利用と更新情報
- 公開ページ: [GitHub Pages](https://itdojp.github.io/kubernetes-cluster-ops-book/)
- リポジトリ: [GitHub](https://github.com/itdojp/kubernetes-cluster-ops-book)
- 更新確認先: [コミット履歴](https://github.com/itdojp/kubernetes-cluster-ops-book/commits/main/)、[Pull Requests](https://github.com/itdojp/kubernetes-cluster-ops-book/pulls)
- Kubernetes 本体、CNI、CSI、Ingress Controller の挙動差は版に依存するため、手順を流用する前に対象クラスタのバージョンと公式ドキュメントを確認してください。
- 本番クラスタで変更を行う前に、メンテナンス計画、etcd バックアップ、ロールバック手順を先に用意してください。
